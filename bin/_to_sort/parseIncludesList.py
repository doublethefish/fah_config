...
def findFileObj(path):
    ...
    if filename not in d_fileObjects:
        raise CFileException(
            "file '{0}' not found in d_fileObjects! fullpath: '{1}'".format(
                filename, path))
...
class CFile:
    ...
    def __str__(self):
        return "{0}:{1}:{2}:dep({3})".format(
            self.filename, self.fullpath, id(self), id(
                self.dependancies))
    ...
    def addDependancy(self, includePath):
        ...
        try:
            ...
            if includePath not in l_ignores:
                warn(
                    "non-project dependancy found : '{0}'".format(includePath))
        ...
        if cur_id == new_obj_id:
            raise Exception(
                "cannot add a file to itself as a dependancy '{0}' == '{1}'".format(
                    self.fullpath, includePath))
    ...
    def getIndirectProjectDependancies(self, doprint):
        ...
        for dependancy in self.dependancies:
            if id(dependancy) == id(self):
                # a raise Exception( "A dependancy has the same id() as its
                # dependee", str(self), id(self), str(dependancy),
                # id(dependancy) )
                continue
            if dependancy.filename == self.filename:
                raise Exception(
                    "A dependancy has the same name as its dependee",
                    str(self),
                    id(self),
                    str(dependancy),
                    id(dependancy))
        ...
        if numDeps != len(self.dependancies):
            raise Exception("self.dependancies has grown by",
                            numDeps - len(self.dependancies), "!")
        return l_ret
    ...
    def getIndirectDependancyCount(self):
        ...
        numDeps = (len(self.getProjectDependancies(False))) + \
            (len(self.externalDependancies))
        return numDeps
    ...
    if commentpos != -1:
        ...
        if len(unparsedIncludeCode2) <= 2:
            print(
                ...
                unparsedIncludeCode +
                "|||" +
                unparsedIncludeCode2 +
                ":::" +
                line)
            return

    containerFile, l_containerPath, containerFilePath = getFilename(
        containerFileFullPath)

    if containerFile not in d_fileObjects:
        raise Exception(
            "Dependee file '{0}' not listed/found in the d_fileObjects struct".format(containerFileFullPath))

    try:
        fileObject = findFileObj(containerFile)
    except CFileException as e:
        warn("Dependant file '{0}' not found in project files, assuming is 3rd party".format(
            containerFileFullPath))
        warn(e)
        return
...
for filename in l_sortedFiles2[:15]:
    ...
    if bExtended:
        print(getTabs() + "All:")
        incDepth()
        deps = fileObject.getProjectDependancies(
            True) + fileObject.externalDependancies[:]
        decDepth()
    bExtended = True
    if bExtended:
        ...
        if deps is None:
            deps = fileObject.getProjectDependancies(
                False) + fileObject.externalDependancies[:]
        for dep in deps:
            idx -= 1
...
for filename in l_sortedFiles3[:15]:
    print(
        "{} objects depend upon {}".format(
            cmph(filename),
            d_fileObjects[filename][0]))
