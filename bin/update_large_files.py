...
def find_matching_files(base_path, exact_match, strings):
    ...
    return [
        os.path.join(
            base_path,
            string) for string in strings if c0 == string[0] and exact_match == string]
...
def sync_potential_git_lfs(git_dir_or_file):
    ...
    # the path where we need to do the install and sync from
    working_dir = os.path.dirname(git_dir_or_file)
