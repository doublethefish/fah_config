...
if args[0].all:
    ...
    for i in range( len(hashes_to_diff) - 1):
        ...
        cmd = ['post-review', '-g', '--branch', upstream_branch, # + ' (' + hashes_to_diff[(i+1)] + ')',
    ...
    review = execute( ['post-review', '-g', '--branch', branch + ' (' + hashes_to_diff[1] + ')',
                      '--revision-range', hashes_to_diff[0] + ':' + hashes_to_diff[1] ] + args[1] )
    print review
